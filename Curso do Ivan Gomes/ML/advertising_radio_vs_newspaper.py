...
#sns.pairplot(publi, x_vars=['radio', 'newspaper'], y_vars='sales', size=3, kind='reg')
graph = sns.pairplot(publi, x_vars=['radio', 'newspaper'], y_vars='sales', size=3, kind='reg')
plt.show(graph)
x_train, x_test, y_train, y_test = train_test_split(x,y,test_size=0.50,random_state=5)
reglin = LinearRegression()
reglin.fit(x_train, y_train)

print(list(zip(['radio', 'newspaper'],reglin.coef_)))
